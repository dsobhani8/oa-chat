...
test('billing checkout uses a minimal account-linked plan flow', () => {
    ...
    assert.equal(html.includes('id="billing-modal"'), true);
    ...
    assert.equal(source.includes("plan.name || 'Premium'"), true);
    assert.equal(source.includes('tickets per month'), true);
    ...
    assert.equal(source.includes('Claim ${ticketCount} tickets'), true);
    assert.equal(source.includes('Retry ticket claim'), true);
    assert.equal(source.includes('getNextClaimTicketCount()'), true);
    assert.equal(source.includes('Math.min(claimableTickets, planTickets)'), true);
    assert.equal(source.includes('claimAndImportTickets(ticketCount'), true);
    assert.equal(source.includes('Preparing tickets...'), true);
    assert.equal(source.includes('Payment complete.'), true);
    assert.equal(source.includes('claimAndImportTickets'), true);
    assert.equal(source.includes('ticketService: this.app.services.tickets'), true);
    assert.equal(source.includes('this.billing.checkout()'), true);
    assert.equal(source.includes('this.billing.portal()'), true);
    assert.equal(source.includes('resetAccountScopedState'), true);
...
test('billing client and demo server use account auth instead of hidden demo email identity', () => {
    ...
    assert.equal(clientSource.includes('async requireAccountAuth()'), true);
    assert.equal(clientSource.includes("this.request('/api/billing/status'"), true);
    assert.equal(clientSource.includes("this.request('/api/billing/checkout'"), true);
    assert.equal(clientSource.includes("this.request('/api/billing/portal'"), true);
    assert.equal(clientSource.includes("this.request('/api/billing/tickets/claim'"), true);
    assert.equal(clientSource.includes('body: { blinded_requests: pairs }'), true);
    assert.equal(clientSource.includes('X-OA-Account-ID'), false);
    assert.equal(clientSource.includes('serializedState'), true);
    assert.equal(clientSource.includes('BILLING_LOCAL_STATE_NOT_SERIALIZABLE'), true);
    assert.equal(clientSource.includes('EMAIL_KEY'), false);
    assert.equal(clientSource.includes('ensureDemoAccount'), false);
    assert.equal(clientSource.includes('stripe-billing-demo'), false);
    assert.equal(clientSource.includes("source: 'stripe-subscription'"), false);

    assert.equal(serverSource.includes('function getAuthenticatedAccountId(req)'), true);
    assert.equal(serverSource.includes('function requireAccount(req)'), true);
    assert.equal(serverSource.includes('function ensureAccountRecord(accountId)'), true);
    assert.equal(serverSource.includes("req.headers['x-oa-account-id']"), false);
    assert.equal(serverSource.includes("url.pathname === '/api/billing/status'"), true);
    assert.equal(serverSource.includes("url.pathname === '/api/billing/checkout'"), true);
    assert.equal(serverSource.includes("url.pathname === '/api/billing/portal'"), true);
    assert.equal(serverSource.includes("url.pathname === '/api/billing/tickets/claim'"), true);
    assert.equal(serverSource.includes("error.code = 'BILLING_ALREADY_ACTIVE';"), true);
    assert.equal(serverSource.includes("metadata[account_id]"), true);
    assert.equal(serverSource.includes('ensureStripeCustomer(account)'), true);
});

test('account billing entitlements are invoice-line idempotent and blind-claimed client-side', () => {
    const appSource = read('chat/app.js');
    const clientSource = read('chat/services/billingClient.js');
    const serverSource = read('scripts/billing-demo-server.mjs');

    assert.equal(appSource.includes('tryRedeemBillingTicketLink(code)'), true);
    assert.equal(clientSource.includes('async claimPremiumTickets'), true);
    assert.equal(clientSource.includes('async claimAndImportTickets'), true);
    assert.equal(clientSource.includes('getOrCreatePendingTicketLinkClaim'), false);
    assert.equal(clientSource.includes('savePendingTicketClaim'), true);
    assert.equal(clientSource.includes('clearPendingTicketClaim(result.accountId)'), true);
    assert.equal(clientSource.includes('assertNoServerFinalizedTickets'), true);
    assert.equal(clientSource.includes('privacyPass.finalizeToken'), true);
    assert.equal(clientSource.includes('serializedState'), true);
    assert.equal(clientSource.includes('finalized_tickets'), true);

    assert.equal(serverSource.includes('function getInvoiceLinePriceId'), true);
    assert.equal(serverSource.includes('line.pricing?.price_details?.price'), true);
    assert.equal(serverSource.includes('function processPendingInvoicesForCustomer(customerId, accountId)'), true);
    assert.equal(serverSource.includes('pendingInvoices: parsed.pendingInvoices || {}'), true);
    assert.equal(serverSource.includes('processedInvoiceLines'), true);
    assert.equal(serverSource.includes('createEntitlementForInvoiceLine'), true);
    assert.equal(serverSource.includes('totalTickets: PREMIUM_TICKETS_PER_PERIOD'), true);
    assert.equal(serverSource.includes('unclaimedTickets: PREMIUM_TICKETS_PER_PERIOD'), true);
    assert.equal(serverSource.includes('function findSpendableEntitlement(accountId, ticketCount)'), true);
    assert.equal(serverSource.includes('function buildClaimId(accountId, blindedRequests)'), true);
    assert.equal(serverSource.includes('async function ensureTicketIssuer()'), true);
    assert.equal(serverSource.includes('publicVerif.Issuer'), true);
    assert.equal(serverSource.includes("url.pathname === '/api/ticket/issue/public-key'"), true);
    assert.equal(serverSource.includes('signed_responses: existing.signedResponses'), true);
    assert.equal(serverSource.includes('replayed: true'), true);
    assert.equal(serverSource.includes("ticket_mode: 'production'"), true);
    assert.equal(serverSource.includes('finalized_tickets'), false);
});
