...
test('billing checkout uses a compact single-plan popup flow', () => {
    ...
    assert.equal(source.includes('clearStoredEmail?.()'), true);
...
test('billing subscription ticket links are email-delivered and redeemed client-side', () => {
    ...
    assert.equal(clientSource.includes('async claimCheckoutSession(sessionId, blindedRequests)'), true);
    assert.equal(clientSource.includes("this.post('/api/billing/checkout-session/claim'"), true);
    assert.equal(clientSource.includes('async redeemCheckoutSession(sessionId, options = {})'), true);
    assert.equal(clientSource.includes('assertCheckoutClaimActive(options)'), true);
    assert.equal(clientSource.includes("error.name = 'AbortError';"), true);
    ...
    assert.equal(clientSource.includes('getOrCreatePendingCheckoutSessionClaim'), true);
    ...
    assert.equal(serverSource.includes("url.pathname === '/api/billing/checkout-session/claim'"), true);
    assert.equal(serverSource.includes('async function handleCheckoutSessionClaim'), true);
    assert.equal(serverSource.includes('pending: true'), true);
    ...
    assert.equal(serverSource.includes('storePendingPaidInvoice(invoice, eventCreated, { accountId })'), true);
...
test('billing demo server reuses pending subscription checkout sessions', () => {
    ...
    assert.equal(source.includes("const pendingCheckout = getReusablePendingCheckout(existingAccount, 'subscription');"), true);
