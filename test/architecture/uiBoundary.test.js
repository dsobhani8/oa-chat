...
test('billing checkout uses a compact single-plan popup flow', () => {
    ...
    assert.equal(html.includes('aria-label="Account"'), true);
    assert.equal(html.includes('<span>Account</span>'), true);
    assert.equal(html.includes('aria-label="Upgrade"'), true);
    assert.equal(html.includes('<span>Upgrade</span>'), true);
    assert.equal(source.includes('fixed inset-0 flex h-screen w-screen'), false);
    assert.equal(source.includes('billing-back-btn'), false);
    ...
    assert.equal(source.includes('Add ${ticketCount} tickets'), false);
    assert.equal(source.includes('Top-up purchases are not configured locally.'), false);
    assert.equal(source.includes("priceLabel: '$10', description: '100 tickets'"), false);
    assert.equal(source.includes('handlePlanAction'), false);
    assert.equal(source.includes("this.handlePlanAction('topup')"), false);
    assert.equal(source.includes('checkoutSubscriptionForAccount(accountId)'), false);
    assert.equal(source.includes('checkoutTopupForAccount(accountId)'), false);
    assert.equal(source.includes('this.billing.checkoutTopup(email)'), false);
    assert.equal(source.includes('this.billing.checkout(email)'), false);
    assert.equal(source.includes('this.account = this.app.services.account'), true);
    ...
    assert.equal(source.includes("this.clearError('status');"), true);
    assert.equal(source.includes("this.setError(message, 'checkout');"), true);
    assert.equal(source.includes('this.open({ skipRefresh: true });'), true);
    assert.equal(source.includes("this.app?.showToast?.(message, 'error', 7000);"), true);
    assert.equal(source.includes('hasBlockingSubscription()'), true);
    assert.equal(source.includes('hasPaidSubscription()'), true);
    ...
    assert.equal(source.includes('if (this.accountState?.isReady)'), true);
    assert.equal(source.includes("this.notice = 'Payment complete. Open Account to recover Premium status and claim tickets.';"), true);
    assert.equal(source.includes('void this.handleReturnedCheckout(pendingSessionId, { fromStoredSession: true });'), true);
    assert.equal(source.includes('checkoutSessionClaimsInFlight'), true);
    assert.equal(source.includes('this.checkoutSessionClaimsInFlight.has(normalizedSessionId)'), true);
    assert.equal(source.includes('this.localResetGeneration = 0;'), true);
    assert.equal(source.includes('const claimGeneration = this.localResetGeneration;'), true);
    assert.equal(source.includes('this.localResetGeneration += 1;'), true);
    assert.equal(source.includes('this.checkoutSessionClaimsInFlight.clear();'), true);
    ...
    assert.equal(source.includes('Payment received. Premium tickets are still being prepared.'), true);
    assert.equal(source.includes('Still waiting for tickets to become claimable'), false);
    assert.equal(source.includes('rememberCheckoutEmail(sessionId)'), false);
    assert.equal(source.includes('checkout(checkoutEmail)'), false);
    ...
    assert.equal(source.includes('billing-claim-btn'), false);
    assert.equal(source.includes('handleTicketDownload()'), false);
    assert.equal(source.includes('getOrCreatePendingTicketClaim(email, requestCount)'), false);
    assert.equal(source.includes('claim.requests.map(request => request.blindedRequest)'), false);
    assert.equal(source.includes('buildTicketPayloadFromClaim({'), false);
    assert.equal(source.includes('downloadTicketPayload(payload)'), false);
    assert.equal(source.includes('clearPendingTicketClaim?.(email)'), false);
    assert.equal(source.includes('addDemoTickets'), false);
    assert.equal(source.includes('Production subscription tickets are not enabled yet.'), false);
    ...
    assert.equal(source.includes('this.visibilityChangeHandler = null;'), true);
    assert.equal(source.includes("document.visibilityState !== 'visible'"), true);
    assert.equal(source.includes("document.addEventListener('visibilitychange', this.visibilityChangeHandler);"), true);
    assert.equal(source.includes("this.busyAction !== 'portal' && this.busyAction !== 'checkout'"), true);
    assert.equal(source.includes("this.busyAction !== 'portal' && this.busyAction !== 'checkout' && this.busyAction !== 'topup'"), false);
    assert.equal(source.includes('ensureBillingAccountStatus()'), false);
    assert.equal(source.includes('ensureDemoAccountEmail'), false);
    assert.equal(source.includes('createAccount(email)'), false);
    assert.equal(source.includes('status?.accountExists === false'), false);
    assert.equal(source.includes('Reset test billing'), false);
    assert.equal(source.includes('Demo controls'), true);
    ...
    assert.equal(source.includes('clearDemoTickets?.()'), true);
    ...
    assert.equal(source.includes('this.statusEmail = null;'), false);
    assert.equal(source.includes('this.statusEmail = status?.billingEmail || status?.email || email;'), false);
    assert.equal(source.includes('this.statusAccountId = accountId;'), true);
    assert.equal(source.includes('getCurrentStatus()'), true);
    assert.equal(source.includes('this.status = null;'), true);
    assert.equal(css.includes('.billing-upgrade-dialog'), true);
    assert.equal(css.includes('@keyframes billing-dialog-in'), true);
    assert.equal(css.includes('@media (max-width: 640px)'), true);
    assert.equal(css.includes('@media (prefers-reduced-motion: reduce)'), true);
    assert.equal(/var\(--(foreground|muted-foreground|primary|primary-foreground|accent|border|background|destructive|card)\)/.test(css), false);
});

test('billing client and demo server expose account-only billing contract', () => {
    ...
    assert.equal(clientSource.includes('ensureDemoAccountEmail()'), false);
    assert.equal(clientSource.includes('async ensureDemoAccount()'), false);
    assert.equal(clientSource.includes('async createAccount(email)'), false);
    assert.equal(clientSource.includes('async checkoutTopup('), false);
    assert.equal(clientSource.includes('async claimCheckoutSession('), false);
    assert.equal(clientSource.includes('async redeemCheckoutSession('), false);
    ...
    assert.equal(serverSource.includes("url.pathname === '/api/billing/topup'"), false);
    ...
    assert.equal(serverSource.includes('getDemoAccountSession(req)'), true);
    assert.equal(serverSource.includes("const DEMO_ACCOUNT_HEADER = 'x-oa-demo-account-id';"), true);
    assert.equal(serverSource.includes('function hasBlockingSubscription'), false);
    assert.equal(debugPage.includes('async function ensureAccountExists(email)'), false);
    assert.equal(debugPage.includes("await post('/api/billing/account', { email });"), false);
    assert.equal(debugPage.includes('await ensureAccountExists(email);'), false);
    assert.equal(debugPage.includes('id="account-id"'), true);
    assert.equal(debugPage.includes("post('/api/billing/checkout', {}, accountId)"), true);
    assert.equal(debugPage.includes("post('/api/billing/portal', {}, accountId)"), true);
    assert.equal(debugPage.includes("post('/api/billing/tickets/claim', {"), true);
    assert.equal(debugPage.includes("'X-OA-Demo-Account-ID'"), true);
...
test('billing subscription ticket links are email-delivered and redeemed client-side', () => {
    ...
    assert.equal(clientSource.includes('async claimCheckoutSession(sessionId, blindedRequests)'), false);
    assert.equal(clientSource.includes("this.post('/api/billing/checkout-session/claim'"), false);
    assert.equal(clientSource.includes('async redeemCheckoutSession(sessionId, options = {})'), false);
    assert.equal(clientSource.includes('assertCheckoutClaimActive(options)'), false);
    assert.equal(clientSource.includes('async redeemCurrentAccountTickets(accountId'), true);
    assert.equal(clientSource.includes("this.post('/api/billing/tickets/claim'"), true);
    assert.equal(clientSource.includes('getPendingAccountTicketClaim(accountId)'), true);
    assert.equal(clientSource.includes('billing_entitlement_id'), false);
    assert.equal(clientSource.includes('billing_invoice_id'), false);
    ...
    assert.equal(clientSource.includes('getOrCreatePendingCheckoutSessionClaim'), false);
    ...
    assert.equal(serverSource.includes("url.pathname === '/api/billing/checkout-session/claim'"), false);
    assert.equal(serverSource.includes('async function handleCheckoutSessionClaim'), false);
    assert.equal(serverSource.includes('function sendTicketLinkClaimResponse'), true);
    ...
    assert.equal(serverSource.includes('storePendingPaidInvoice(invoice, eventCreated, { accountId })'), false);
    assert.equal(serverSource.includes('storePendingPaidInvoice(invoice, eventCreated);'), true);
    assert.equal(serverSource.includes('const customer = email ? ensureAccountRecord(email) : null;'), true);
    ...
    assert.equal(serverSource.includes('processPendingPaidInvoicesForCustomer(customerId, email || account.billingEmail, accountId)'), true);
    ...
    assert.equal(serverSource.includes('const legacyClaimId = buildClaimId(link.email, blindedRequests);'), false);
    assert.equal(/function buildTicketLinkClaimId\(link, blindedRequests\)[\s\S]*?email: link\.email/.test(serverSource), false);
    assert.equal(serverSource.includes('retryDuplicateInvoicePaidDelivery(event)'), true);
    assert.equal(serverSource.includes("link.deliveryMethod === 'console-fallback'"), true);
    assert.equal(serverSource.includes('targetEntitlementId || !hasTicketLinkForEntitlement(entitlement.id)'), false);
    assert.equal(serverSource.includes('getManualClaimableTicketCount(entitlement)'), false);
    assert.equal(serverSource.includes('function hasTicketLinkForEntitlement(entitlementId)'), false);
...
test('billing demo server reuses pending subscription checkout sessions', () => {
    ...
    assert.equal(source.includes('function maybeCreateDemoRenewalEntitlement(accountId)'), true);
    assert.equal(source.includes("sourceType: 'subscription_demo_renewal'"), true);
    assert.equal(source.includes('demoRenewalPeriodIndex'), true);
});
