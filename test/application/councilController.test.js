...
test('requestLaneAccess accepts a non-verified proof only in explicit loopback mode', async () => {
    const savedSessions = [];
    const verifierRegistrations = [];
    const controller = createController({
        ticketCount: 1,
        costs: { 'openai/gpt': 1 },
        chatDB: {
        ...
        inferenceService: {
            ...
            setCurrentAccess: (_session, accessInfo) => verifierRegistrations.push(accessInfo)
        }
    });
    ...
    assert.equal(verifierRegistrations.length, 0);
    assert.equal(savedSessions.length, 1);
});

test('requestLaneAccess fails closed for every non-verified production result', async () => {
    for (const status of ['pending', 'unverified', 'rejected', 'unknown']) {
        const savedSessions = [];
        const controller = createController({
            ticketCount: 1,
            costs: { 'openai/gpt': 1 },
            ...
            inferenceService: {
                ...
                verifyAccess: async () => ({ status, detail: `${status}-detail` }),
                setCurrentAccess: () => {
                    throw new Error('unverified access must not be registered');
                }
            }
        });
        ...
        assert.deepEqual(session.councilAccess, {});
        assert.equal(savedSessions.length, 0);
    }
});

test('rejected Council access cannot leak a provisional raw key into session mappings', async () => {
    const originalMappings = {
        existing: {
            underlyingKeyId: 'existing-approved-key',
    ...
    const controller = createController({
        ...
        inferenceService: {
            getAccessLabel: () => 'OpenRouter key',
            requestAccess: async () => ({
                key: 'rejected-provisional-key',
                stationId: 'station-a',
                expiresAt: new Date(Date.now() + 60_000).toISOString()
            }),
            getVerificationAdapter: () => ({ supports: true }),
            verifyAccess: async () => ({ status: 'rejected', detail: 'not-approved' }),
            setAccessInfo: () => {
                throw new Error('provisional Council access must remain detached');
            },
            setCurrentAccess: () => {
                throw new Error('rejected access must not be registered');
            }
        }
    });
    const session = {
        id: 'session-rejected-mapping',
        councilAccess: {},
        ephemeralKeyMappings: originalMappings
    };
    ...
    assert.equal(session.ephemeralKeyMappings, originalMappings);
    assert.deepEqual(session.ephemeralKeyMappings, {
        existing: {
            underlyingKeyId: 'existing-approved-key',
            backendId: 'openrouter'
        }
    });
    assert.doesNotMatch(JSON.stringify(session), /rejected-provisional-key/);
});

test('persisted local Council access becomes unusable when loopback policy is off', () => {
    let allowLocalBypass = true;
    const controller = createController({
        inferenceService: {
            getVerificationAdapter: () => ({
                supports: true,
                allowsLocalBypass: () => allowLocalBypass
            })
        }
    });
    const entry = { laneId: 'primary', id: 'openai/gpt', name: 'GPT' };
    const session = {
        councilAccess: {
            primary: {
                apiKey: 'local-lane-key',
                apiKeyInfo: {
    ...
    assert.equal(controller.needsFreshLaneAccess(session, entry), false);
    allowLocalBypass = false;
    assert.equal(controller.needsFreshLaneAccess(session, entry), true);
});
